...
namespace myna {
    ...
    Mat<T>::Mat(const Mat<T> &mat) {
        ...
        //memcpy(this->data, dataType.data, (this->cols*this->rows) * sizeof(T));
        this->data = new T *[this->rows];
        for (int i = 0; i < this->rows; i++) {
            this->data[i] = new T[this->cols];
        }
    ...
    MATTEMPLATE
    Mat<T> Mat<T>::matAddVec(const Mat<T> &vec) {
        assert(vec.getRows() == 1 || vec.getCols() == 1);
        Mat<T> B(this->rows, this->cols);
        if (vec.getRows() == this->rows) {
            ...
            throw "Vector's dimension does not match the matrix!";
        }
    }
